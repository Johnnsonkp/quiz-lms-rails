...
function TableOfContents({ quiz_preview }: { quiz_preview: QuizPreview[] | null }) {
  return (
    <div className="border-2 border-[#E5E7EB] rounded-md w-full bg-[#F9FAFB] shadow-sm">
      <h4 className="font-semibold border-b border-[#F3F4F6] py-2 px-2 text-xs">Table of Contents</h4>
        ...
        {quiz_preview && quiz_preview.length > 0 && quiz_preview.map((quiz, index) => (
          <li key={index} className="border border-gray-200 transition-shadow duration-200 hover:bg-[#F3F4F6]">
            <details className="group">
              <summary className="text-[11px] flex items-center justify-between gap-2 p-2 font-medium marker:content-none hover:cursor-pointer">
                  ...
                  <ul className="flex flex-col gap-1 pl-2 ">
                    {quiz?.quiz_details?.map((detail, idx) => (
                      <div key={idx} className="flex items-center gap-1">
                        ...
                        <li key={idx} className="cursor-pointer border-t py-1 border-gray-200 text-[10px] text-gray-600 hover:underline">
                          {detail?.title}
                        </li>
                      </div>
                      ))}
